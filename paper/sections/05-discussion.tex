...
\subsection{Relationship to DMMF}

This work extends the Developer Mental Model Framework \cite{silva2025dmmf} in two directions:

\subsubsection{Temporal Extension}

DMMF analyzed historical artifacts (commits over 3 years) to infer cognitive patterns. Cyborg Developer analyzes real-time interactions (sessions over 30 days) to observe cognitive distribution. Together, they provide complementary views:

\begin{itemize}
    \item \textbf{DMMF}: What cognitive patterns produce software artifacts?
    \item \textbf{Cyborg Developer}: How are cognitive processes distributed in artifact production?
\end{itemize}

\subsubsection{Cognitive Extension}

DMMF's four dimensions (cognitive, affective, conative, reflective) describe the \textit{individual} developer's mental model as inferred from artifacts. Cyborg Cognition's four dimensions (delegation, extension, selection, fluidity) describe the \textit{human-AI system's} cognitive distribution as observed in real-time interactions.
...
\begin{table*}[h]
\caption{DMMF vs Cyborg Cognition: Complementary Levels of Analysis}
\label{tab:frameworks}
\centering
\begin{tabular}{lll}
\toprule
\textbf{DMMF (Individual)} & \textbf{Cyborg (System)} & \textbf{Relationship} \\
\midrule
...
The key distinction: DMMF captures \textit{what} cognitive patterns produce artifacts; Cyborg Cognition captures \textit{how} cognitive work is distributed during artifact production. Together, they provide complementary lenses---one retrospective (artifact analysis), one concurrent (interaction analysis).
...
\begin{itemize}
    ...
    \item \textbf{Cultural and linguistic context}: A Brazilian developer working primarily in Portuguese on Brazilian government transparency projects represents a specific context. Collaboration patterns may differ in other linguistic, cultural, or domain contexts.

    \item \textbf{Selection bias in projects}: The cidadao.ai ecosystem dominates the dataset (62\% of sessions). Findings may reflect this project's characteristics rather than general development patterns.
...
\begin{itemize}
    ...
    \item \textbf{Tool evolution}: Claude Code's capabilities and interfaces evolve rapidly. Patterns observed with the December 2024--January 2025 version may not replicate with future versions.
